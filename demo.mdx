...
In this example, we will build a Hamming test agent **taking on the role of the customers** to evaluate how well the virtual voice agent schedules appointments for Sam's customers. 

### Components to Test your Virtual Voice Agent
...
<CardGroup>
  ...
  title="Datasets" 
  icon="square-1">
  Datasets are a collection of customer phone calls/scenarios that you want our Hamming agent to simulate. You can upload an existing dataset in 
  ...
  icon="square-2">
 To evaluate your virtual voice agent you need to create prompts defining the task and success criteria for each call. This is critical to ensure your virtual voice agent is evaluated only on the things that matter to your business.
  </Card>

  <Card 
  title="Scorer" 
  icon="square-3">
  The scorer is a tool used to assess the performance of the virtual voice agent when talking to the Hamming test agent. It evaluates the agent's responses by comparing them to the scoring prompt. Imagine it like a **mediator** or **observer**.
  </Card>

<Card 
  title="Hamming Test Agent" 
  icon="square-4">
  To test your virtual voice agent, the Hamming test agent will simulate multiple interactions using the scenarios provided in your dataset. Remember, the Hamming test agent **takes on the role of your customers**!
  </Card>
...
<Steps>
  <Step title= "Create A Dataset">
    ...
    2. Click on **+Add New Dataset**. You can either upload an existing dataset or create an empty dataset and then use the JSON editor to build your dataset.
       The dataset should contain **all phone calls/scenarios** you'd like the Hamming's test agent to simulate. Since each business is 
    ...
    4. Select the input columns. The input columns are the data points you'd the Hamming test agent to reference.
...
<iframe src="https://drive.google.com/file/d/1NgsQL6KXKV3_Dh0f2JbtdW8_9iMSNCuA/preview" width="100%" height="auto" allowfullscreen></iframe>
 ...
 <Step title="Create A Scoring Prompt (aka Observer)" >
    1. Visit [app.hamming.ai/prompts](http://app.hamming.ai/prompts) to create a scoring prompt.
       The prompt should contain concise instructions for the Observer.
    ...
    **[TASK]** This describes the task to Hamming's test agent _e.g. Analzye the call between a receptionist and a customer_
    **[CRITERIA]** This defines what makes the call successful. _e.g. Was the virtual voice agent able to schedule an appointment?      1 - Yes; 0 - No;_
...
**Pro Tip**: Ensure that the values in the scorer align with the labels from your scoring prompt. For example, if the value "0" represents "No" in your prompt, make sure the scorer also labels "0" as "No."
...
<Step title="Create A Hamming Test Agent">
    ...
    5. Configure a prompt for the Hamming test agent with input variables from your dataset, allowing it to simulate a 
       variety of customer situations. This prompt should be instructions for the Hamming test agent calling the 
       dealership. 
    6. Select the dataset you uploaded in Step 1.
    7. Click on Run and enter the phone number of your virtual voice agent. You can set up a phone number
       on Retell AI. 

<Tip>

**Pro Tip**: If you want to speak to Hamming's test agent, you can enter your own phone number and Mr. Hamming will call you.
