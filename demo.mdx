...
<Cards columns=4>
  ...
  <Card title="Dataset" icon="📊">
  Datasets are a collection of phone calls that you want to test. You can upload an existing dataset in CSV or JSON format or create a new 
  dataset using JSON.
  </Card>

  <Card title="Prompts" icon="🚀">
  Prompts are probably the most important elements of running a test. To test a voice agent you need to create two different prompts. 
The first prompt consists of instructions for the test agent when evaluating the call. The second prompt are instructions for the customer when running the test calls.
  </Card>

  <Card title="Scorer" icon="🚀">
  The scorer is a function using the prompt to evaluate the responses and label them. 
  </Card>

<Card title="Voice Agent" icon="🚀">
  The scorer is a function using the prompt to evaluate the responses and label them. 
  </Card>
