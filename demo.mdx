...
<CardGroup>
  ...
  title="Datasets" 
  icon="square-1">
  Datasets are a collection of phone calls/scenarios that you want to test. You can upload an existing dataset in CSV or JSON format or create a new 
  dataset using JSON.
  </Card>
  ...
  title="Prompts" 
  icon="square-2">
 To test a voice agent you need to create two different prompts. The first prompt consists of instructions for the test 
 agent when evaluating the call. The second prompt consists of instructions for the customer when running the calls.
  </Card>

  <Card 
  title="Scorer" 
  icon="square-3">
  The scorer is a tool used to assess the performance of a voice agent. The scorer evaluates the test agent's responses by comparing them to the expected outputs generated from the test prompt, ensuring accuracy and relevance.
  </Card>

<Card 
  title="Voice Agents" 
  icon="square-4">
  To test your voice agent you can run mutliple calls against your agent and see how well it performs.
  </Card>
